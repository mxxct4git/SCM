						...
						<div class="col-md-12">
							<div class="box box-primary">
								<div class="box-body">
									<div class="row">
										<div class="col-md-8">
											<form class="form-horizontal" action="/cost/queryCost" method="post">
												<div class="box-body">
							...
							<div class="box box-primary">
								...
								<div class="box-body table-responsive">
									<table id="example1" class="table table-bordered table-striped">
										...
										<tr>
											<td>${item.id}</td>
											<td>${item.inlist_timestamp}</td>
											<td>${item.in_date}</td>
											<td>${item.cost_total}</td>
											<td>
                                            <#if item.is_cost_complete == 1>
                                                已完结
                                            <#else>
                                                未完结
                                            </#if>
											...
											<td style="text-align: center;"><a href="/cost/detailCost?id=${item.id}" target="_blank"><i class="fa fa-file-o"></i></a></td>
										</tr>
                                        </#list>
										</tfoot>
									</table>
		...
		<script>
            ...
            function checkById(id) {
                if (confirm("确定要结算该条成本吗？")) {
                    location.href = "/cost/check?id=" + id;
                }
            };
            function checkById(id) {
                if (confirm("确定要取消结算该条成本吗？")) {
                    location.href = "/cost/uncheck?id=" + id;
                }
            }
		</script>
        <script>
